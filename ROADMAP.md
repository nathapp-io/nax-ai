...
2. **The replay suite's tool-call excusal branch is currently untested.** `test/protocols/replay.test.ts` excuses "emitted no text delta" when a fixture contains a tool call, since a pure tool-call turn legitimately has no prose. The recorded `opencode-go-anthropic-messages-tool.json` fixture, despite its name, contains no actual tool-call events — the model declined to call the tool during recording and only produced text — so this excusal branch has no fixture exercising it. Not a bug; closing it needs a future re-recording that successfully elicits a tool call.
