// test/protocols/openai-codex-responses.test.ts
import { describe, expect, it } from "vitest";
import {
  createOpenAiCodexResponsesPi,
  type PiStreamEvent,
} from "../../src/protocols/openai-codex-responses/backend-pi.ts";
...
describe("openai-codex-responses pi backend", () => {
  it("maps text events to text deltas", async () => {
    const protocol = createOpenAiCodexResponsesPi({ client: fakePi(TEXT_EVENTS) });
  ...
  it("accumulates streamed tool arguments and emits one parsed tool call", async () => {
    const protocol = createOpenAiCodexResponsesPi({ client: fakePi(TOOL_EVENTS) });
  ...
  it("emits an error event when tool arguments do not parse", async () => {
    ...
    const protocol = createOpenAiCodexResponsesPi({
      client: fakePi([
  ...
  it("passes the system prompt as instructions, not as a message", async () => {
    ...
    const protocol = createOpenAiCodexResponsesPi({ client: fakePi(TEXT_EVENTS, capture) });
...
runProtocolConformance(
  "openai-codex-responses (pi)",
  async () => createOpenAiCodexResponsesPi({ client: fakePi(TEXT_EVENTS) }),
  {
...
runProtocolConformance(
  "openai-codex-responses (pi, tools)",
  async () => createOpenAiCodexResponsesPi({ client: fakePi(TOOL_EVENTS) }),
  {
