// test/protocols/openai-completions.test.ts
import { describe, expect, it } from "vitest";
import { createOpenAiCompletionsPi, type PiStreamEvent } from "../../src/protocols/openai-completions/backend-pi.ts";
...
describe("openai-completions pi backend", () => {
  it("maps text events to text deltas", async () => {
    const protocol = createOpenAiCompletionsPi({ client: fakePi(TEXT_EVENTS) });
  ...
  it("accumulates streamed tool arguments and emits one parsed tool call", async () => {
    const protocol = createOpenAiCompletionsPi({ client: fakePi(TOOL_EVENTS) });
  ...
  it("emits an error event when tool arguments do not parse", async () => {
    ...
    const protocol = createOpenAiCompletionsPi({
      client: fakePi([
...
runProtocolConformance(
  ...
  async () => createOpenAiCompletionsPi({ client: fakePi(TEXT_EVENTS) }),
  { text: { name: "text", request: TEXT_REQUEST } },
);

runProtocolConformance(
  "openai-completions (pi, tools)",
  async () => createOpenAiCompletionsPi({ client: fakePi(TOOL_EVENTS) }),
  {
